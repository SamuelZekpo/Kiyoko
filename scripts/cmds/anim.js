  ...
  onStart: async function ({ api, message, event }) {
    try {
      if (!event.messageReply?.attachments?.[0]?.url) {
        return message.reply("âŒ Please reply to an image!");
      }

      const imgUrl = event.messageReply.attachments[0].url;
      const processingMsg = await message.reply("ðŸ”„ Creating anime art...");
      ...
      await message.reply({
        body: "ðŸŽ¨ Your anime transformation is ready!",
      ...
      message.reply("âŒ Failed to create anime art. Please try another image.");
    }
  }
};
